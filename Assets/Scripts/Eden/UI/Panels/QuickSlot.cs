...
namespace Eden.UI.Panels {
	...
	public class QuickSlot: Dumpster.Core.BuiltInModules.UI.Panel {
		...
		private void Clear () {
			...
			foreach( ItemBubbleUI item in _items ) {
				if ( item != null ) {
		...
		private void ShiftItemsLeft () {
			...
			_items = ArrayMath.ShiftArrayLeft<ItemBubbleUI>( _items );	
		}
		private void ShiftItemsRight () {

			_items = ArrayMath.ShiftArrayRight<ItemBubbleUI>( _items );
		}
		private void Move () {
		...
		private Item ItemForIndex ( int index ) {
		...
		private ItemBubbleUI CreateItem ( Item item, int index ) {
